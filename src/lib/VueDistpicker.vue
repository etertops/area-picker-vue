<template>
  <div class="address_ctx">
    <div class="address_show">
      <div class="address_show_items">
        <span v-for="(x, i) in selectAddress" @click="selectItems(i)">
          <span :class="{'show_selected': i === showLevel - 1}">{{x.address}}</span>
          <span v-if="i < selectAddress.length - 1">&ensp;/&ensp;</span>
        </span>
      </div>
      <div class="address_show_block" @click="selectItems(0)"></div>
    </div>
    <div class="address_selcet_out"  v-if="showLevel > 0">
      <div class="address_selcet">
        <div class="address_selcet_nav">
          <div class="address_selcet_nav_item" @click="selectItems(0)" :class="{'address_selcet_nav_item_on': showLevel === 1}">省份</div>
          <div class="address_selcet_nav_item" @click="selectItems(1)" :class="{'address_selcet_nav_item_on': showLevel === 2}">城市</div>
          <div class="address_selcet_nav_item" @click="selectItems(2)" :class="{'address_selcet_nav_item_on': showLevel === 3}">区县</div>
          <div class="address_selcet_nav_item_block"></div>
        </div>
        <div class="address_selcet_ctx">
          <div class="address_selcet_ctx_item" v-if="showLevel === 1">
            <div class="address_selcet_ctx_province" v-for="(item, key) in provinceData">
              <div class="address_selcet_ctx_province_lab">{{key}}</div>
              <div class="address_selcet_items_ctx">
                <div class="address_selcet_item" v-for="x in item" @click="selectProvince(x)" :class="{'selected': x.address === currProvince}">{{x.address}}</div>
              </div>
            </div>
          </div>
          <div class="address_selcet_ctx_item" v-if="showLevel === 2">
            <div class="address_selcet_items_ctx">
              <div class="address_selcet_item" v-for="(value, key) in cityData" @click="selectCity(value, key)" :class="{'selected': value === currCity}">{{value}}</div>
            </div>
          </div>
          <div class="address_selcet_ctx_item" v-if="showLevel === 3">
            <div class="address_selcet_items_ctx">
              <div class="address_selcet_item" v-for="(value, key) in countyData" @click="selectCounty(value, key)" :class="{'selected': value === currCounty}">{{value}}</div>
            </div>
          </div>
        </div>
      </div>
      <div style="flex: 1" @click="clickOut"></div>
    </div>
  </div>
</template>
...
let cityData = {
    ...
    441200: {441202: '端州区', 441203: '鼎湖区', 441223: '广宁县', 441224: '怀集县', 441225: '封开县', 441226: '德庆县', 441283: '高要区', 441284: '四会市'},
  ...
  methods: {
    ...
    selectProvince (obj) {
      ...
      if (this.showLevel < this.level) {
        ...
        this.showLevel = 0
        this.$emit('finish', this.selectAddress)
      }
    },
    selectCity (city, code) {
      this.currCity = city
      this.selectAddress.splice(1, this.selectAddress.length)
    ...
    initAddress () {
      let me = this
      me.currProvince = me.province
      let province = null
      for (let key in me.provinceData) {
        let flag = false
        for (let i in me.provinceData[key]) {
          if (me.provinceData[key][i].address === me.province) {
            province = me.provinceData[key][i]
            flag = true
            break
          }
        }
      ...
      me.selectAddress.splice(0, me.selectAddress.length)
      me.selectAddress.push({
      ...
      if (me.level >= 2) {
        ...
        if (citys !== null) {
          ...
          for (let key in cityData) {
            if (me.selectAddress[1].code === key) {
              countys = cityData[key]
            }
          }
        }
      }
      if (me.level >= 3) {
        me.currCounty = me.county
        if (countys != null) {
          for (let key in countys) {
            if (me.county === countys[key]) {
              me.selectAddress.push({
                county: countys[key],
                code: key,
                address: countys[key]
...
<style>
  ...
  .address_selcet_ctx_item{
    overflow: hidden;
    width: 100%;
  }

  .address_selcet_ctx_province{
    width: 100%;
    overflow: hidden;
    display: flex;
    flex-direction: row;
  ...
  .address_selcet_ctx_province_lab{
    padding: 0 10px;
    height: 23px;
    line-height: 23px;
  }

  .address_selcet_items_ctx{
    flex: 1;
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
  ...
  .show_selected{
    background-color: rgba(17, 255, 199, 0.1);
  }
